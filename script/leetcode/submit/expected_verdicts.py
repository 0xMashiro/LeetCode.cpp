#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""读 source 文件里的 `// @expected: <verdict>` 注释，按 registerStrategy 顺序
映射到 (1-indexed strategy 序号 → LeetCode 全称状态)。

没写注释的解默认 `Accepted`。缩写 TLE/WA/RE/MLE/CE/AC 自动展开成 LeetCode 判题
返回的完整字符串，这样多解验证时 actual.status == expected 直接字符串比对。
"""
...
def parse_expected_verdicts(source: str) -> Dict[int, str]:
    ...
    每个 `registerStrategy` 分别找它的注释，两种位置都接受：
      1) 注释与 register 在**同一行**（trailing）—— `regStrat(...);  // @expected: TLE`
      2) 注释在 register 上一行**顶层独立成行**（leading）
    """
    lines = source.splitlines()
    register_line_indices = [
        (i, m.start()) for i, line in enumerate(lines)
        for m in [_REGISTER_RE.search(line)] if m
    ]
    if not register_line_indices:
        return {}

    result: Dict[int, str] = {}
    for seq, (line_idx, _) in enumerate(register_line_indices, start=1):
