...
class LeetCodeSubmitter:
    ...
    def submit_problem(self, problem_id: int, solution_num: int = 1) -> bool:
        ...
        log_with_time(f"💾 转换后的代码已保存: {submit_file}")
        ...
        while time.time() - start_time < max_wait:
            try:
                ...
                with urllib.request.urlopen(req, timeout=10) as response:
                    ...
                    dots = (dots + 1) % 4
                    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    print(f"\r[{timestamp}] ⏳ 判题中{'.' * dots}{' ' * (3-dots)}", end='', flush=True)
                    time.sleep(1)
...
请输出转换后的代码："""
    ...
    def _submit_to_leetcode(self, problem_data: ProblemData, code: str) -> bool:
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=30) as response:
                ...
                if not submission_id:
                    log_with_time(f"❌ 提交失败: {result}", ColorCode.RED)
                    return False
            ...
            log_with_time(f"❌ 提交异常: {e}", ColorCode.RED)
            return False
    ...
    def _wait_for_result(self, submission_id: str, max_wait: int = 60) -> bool:
        ...
        log_with_time("⏱️  等待超时", ColorCode.YELLOW)
        return False
    ...
    def _display_result(self, result: Dict) -> bool:
        ...
        if status_code == 10:  # Accepted
            ...
            log_with_time(f"   执行用时: {result.get('status_runtime', 'N/A')}")
            log_with_time(f"   内存消耗: {result.get('status_memory', 'N/A')}")
            return True
        
        elif status_code == 11:  # Wrong Answer
            log_with_time("❌ Wrong Answer", ColorCode.RED)
            log_with_time(f"\n输入:")
            log_with_time(result.get('input', 'N/A'))
            log_with_time(f"\n输出:")
            log_with_time(result.get('code_output', 'N/A'))
            log_with_time(f"\n期望:")
            log_with_time(result.get('expected_output', 'N/A'))
            return False
            ...
            log_with_time(f"错误信息: {result.get('full_runtime_error', 'N/A')}")
            return False
        
        else:
            log_with_time(f"❌ {status_msg}", ColorCode.RED)
            log_with_time(f"详情: {result}")
            return False
